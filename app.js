...
const rewards = {
  ...
  weekly: [
    ...
    "Free time 30 min",
    "Chill Session",
    "Can edit long project"
  ],

  monthly: [
    "Can watch 2",
    "Play games for 30 min",
    "YouTube shorts (no limit)",
    "Enjoy freedom"
  ],
...
const punishments = [
  ...
  "Revise a weak topic",
  "30-minute extra workout",
  "Read for 1 hour",
  "No shorts videos tonight"
];
...
const el = {
  ...
  document.getElementById("selectedCategoryChip"),
  ...
  punishPopup:
  document.getElementById("punishPopup"),
  ...
  punishText:
  document.getElementById("punishText"),


  rewardOkBtn:
  document.getElementById("rewardOkBtn"),
  ...
  sectionHint:
  document.getElementById("sectionHint"),
...
function getRandomRewards(category){

 const list = [...rewards[category]];
 ...
 while(selected.length < amount && list.length){
...
function setCategory(cat){
 ...
 el.selectedCategoryChip.textContent =
 cat.charAt(0).toUpperCase()+cat.slice(1);


 el.customDaysWrap.classList.toggle(
 "hidden",
 cat!=="custom"
 );
...
function renderStats(){
 ...
 el.monthlyCount.textContent =
 milestones.filter(
 m=>m.category==="monthly"
 ).length;


 el.customCount.textContent =
 milestones.filter(
 m=>m.category==="custom"
 ).length;
